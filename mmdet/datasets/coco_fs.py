...
class FewShotCocoDataset(BaseFewShotDataset, CocoDataset):
    ...
    def prepare_train_img(self, idx):
        ...
        env_id = self.env_id
        results = dict(img_info=img_info, ann_info=ann_info, env_id=env_id)
    ...
    def prepare_test_img(self, idx):
        ...
        img_info = self.data_infos[idx]
        env_id = self.env_id
        results = dict(img_info=img_info, env_id=env_id)
        if self.proposals is not None:
            results['proposals'] = self.proposals[idx]
        self.pre_pipeline(results)
        return self.pipeline(results)
...
class FewShotCocoDefaultDataset(FewShotCocoDataset):
    ...
    coco_benchmark = {
        f'{shot}SHOT': [
            dict(
                type='ann_file',
                ann_file=f'/nfs/public_data/few_shot_ann/coco/benchmark_{shot}shot/'
    ...
    DEFAULT_ANN_CONFIG = dict(
        ...
        Attention_RPN={
            **coco_benchmark, 'Official_10SHOT': [
                dict(
                    type='ann_file',
                    ann_file='/nfs/public_data/few_shot_ann/coco/attention_rpn_10shot/'
