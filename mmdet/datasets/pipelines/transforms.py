...
# from mmdet.core.bbox.transforms import find_clip_bboxes
...
class MixUp:
    ...
    def _mixup_transform(self, results):
        ...
        mixup_img = self.mix_ratio[0] * ori_img + self.mix_ratio[1] * padded_cropped_img.astype(np.float32)
...
class RandomAffine2:
    ...
    def __init__(self,
                 ...
                 bbox_clip_border=True,
                 skip_filter=True,
                 filter_split=False, # 是否对切到的过滤
                 test_times=40 # 是否对切到的过滤
                 ):
        assert 0 <= max_translate_ratio <= 1
        assert scaling_ratio_range[0] <= scaling_ratio_range[1]
        assert scaling_ratio_range[0] > 0
    ...
    def __call__(self, results):
        for i in range(self.test_times):
            cp = copy.deepcopy(results)
            re = self.try_affine(cp)
            if re is not None:
                return re
        print('bad affine ')
        return results

    def try_affine(self,results):
        img = results['img']
        height = img.shape[0] + self.border[0] * 2
        width = img.shape[1] + self.border[1] * 2
        # 默认设置 -1/2 的话，就是masaic 1/2的中心点 ，？
        # Rotation
        ...
        for key in results.get('bbox_fields', []):
            ...
            if num_bboxes:
                ...
                if self.filter_split:
                    is_clipbox = find_clip_bboxes(warp_bboxes, height, width)
                    if is_clipbox : # 切到box了
                        return None
                if self.bbox_clip_border:   #bug
                    warp_bboxes[:, [0, 2]] = \
                        warp_bboxes[:, [0, 2]].clip(0, width)
                    warp_bboxes[:, [1, 3]] = \
                        warp_bboxes[:, [1, 3]].clip(0, height)
...
def find_clip_bboxes(bboxes, img_h, img_w):
    ...
    """
    out_inds = (bboxes[:, 2] < 0) | (bboxes[:, 0] > img_w) | (bboxes[:, 3] < 0) | (bboxes[:, 1] > img_h)
    in_inds  = (bboxes[:, 0] >= 0) & (bboxes[:, 2] <= img_w) & (bboxes[:, 1] >= 0) & (bboxes[:, 3] <= img_h)
    if sum(out_inds + in_inds) < len(in_inds):
        return True
    return False