...
class GaussianBlur():
    ...
    def __init__(self, maxv = 2.0):
        self.maxv = maxv

    def __call__(self, results):
        img = results['img']
        sigma = np.random.uniform(0.1, self.maxv)
        if isinstance(img, Image.Image):
            pass
        elif isinstance(img, numpy.ndarray):
            img = Image.fromarray(img)
        else:
            print("error type ", type(img))

        newimg = img.filter(ImageFilter.GaussianBlur(radius=sigma))
...
from .aug_tools import process_mask_poin_aug,processimage
import numpy as np
from PIL import Image
@PIPELINES.register_module()
class RandomImgAugFromAPI(object):
    def __init__(self,type_seq='seq_nocrop'):
        self.type = type_seq
    ...
    def __call__(self, results):
        ...
        for key in results.get('bbox_fields', []):
            ...
            if key == 'gt_bboxes':
                break
        boxs = results['gt_bboxes']
        labels = results['gt_labels']
        image_aug, boxs,labels = processimage(img, boxs,labels=labels, type=self.type)

        newimg = np.array(image_aug)
        results['img'] = newimg
        results['gt_bboxes'] = boxs
