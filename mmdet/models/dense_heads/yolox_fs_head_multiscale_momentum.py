...
class MultiScaleWindowAttetnionLayer(nn.Module):
    def __init__(self, embed_dim=128, nhead=8, ffn_dim=512, window_size_q=[4, 4],
                 window_size_ks=[[4, 4], [2, 2], [1, 1]], dropout=0.1):
        super().__init__()
        self.window_size_q = window_size_q
        self.window_size_ks = window_size_ks
        self.avgpool_q = nn.AdaptiveAvgPool2d(output_size=window_size_q)
        # self.avgpool_k = nn.AvgPool2d(kernel_size=window_size2)
        self.avgpool_ks = nn.ModuleList()
        for i in range(len(window_size_ks)):
            self.avgpool_ks.append(nn.AdaptiveAvgPool2d(output_size=window_size_ks[i]))
        self.softmax = nn.Softmax(dim=-1)
        self.nhead = nhead
    ...
    def forward(self, x1, x2):
        ...
        # x1g = x1 #B, C1, H1p, W1p
        # x2g = x2 #B, C2, H2p, W2p
        q_s = self.avgpool_q(x2)
        qg = self.avgpool_q(x2).permute(0, 2, 3, 1).contiguous()
        qg = qg + self.pos_enc(qg)
        qg = qg.view(B, -1, C2)
        kgs = []
        for avgpool in self.avgpool_ks:
            kg_tmp = avgpool(x1).permute(0, 2, 3, 1).contiguous()
            kg_tmp = kg_tmp + self.pos_enc(kg_tmp)
            kg_tmp = kg_tmp.view(B, -1, C1)
            kgs.append(kg_tmp)
        ...
        attn = self.softmax(attn)
        qg = (attn @ vg).transpose(1, 2).reshape(B, num_window_q, C1)
        qg = qg.transpose(1, 2).reshape(B, C1, self.window_size_q[0], self.window_size_q[1])
        # qg = F.interpolate(qg, size=(H1p, W1p), mode='bilinear', align_corners=False)
        q_s = q_s + qg
        q_s = q_s + self.mlp(q_s)
        q_s = F.interpolate(q_s, size=(H1, W1), mode='bilinear', align_corners=True)
    ...
    def __init__(self, in_channel1=128, in_channel2=128, nhead=8, layers=3, embed_dim=128, ffn_dim=512,
        ...
        for i in range(layers):
            self.attentions.append(
                ...
                #                     window_size1=window_size1, window_size2=window_size2, dropout=0.1)
                MultiScaleWindowAttetnionLayer(embed_dim=embed_dim, nhead=nhead, ffn_dim=ffn_dim, window_size_q=window_size1, 
                window_size_ks=window_size2, dropout=0.1)
            )
...
class YOLOX_FS_Head_MulScale_Mom(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        self.clusters = []
        self.clusters_momentum = []
        for i, k in enumerate(keys):
            params = nn.ParameterDict({})
            clus = torch.load(cluster[k])
            params_m = nn.ParameterDict({})
            for k2 in clus.keys():
                params[k2] = nn.Parameter(clus[k2], requires_grad=False)
                params_m[k2] = nn.Parameter(torch.normal(mean=0, std=0.001, size=clus[k2].size()), requires_grad=True)
            setattr(self, 'clus_{}'.format(i), params)
            self.clusters.append(getattr(self, 'clus_{}'.format(i)))
            setattr(self, 'clus_m_{}'.format(i), params_m)
            self.clusters_momentum.append(getattr(self, 'clus_m_{}'.format(i)))
    ...
    def init_weights(self):
        super(YOLOX_FS_Head_MulScale_Mom, self).init_weights()
    ...
    def forward_train(self,
        ...
        c0 = torch.stack([self.clusters[id]['feats1'] for id in env_id], dim=0) + \
             torch.stack([self.clusters_momentum[id]['feats1'] for id in env_id], dim=0)
        c1 = torch.stack([self.clusters[id]['feats2'] for id in env_id], dim=0) + \
             torch.stack([self.clusters_momentum[id]['feats2'] for id in env_id], dim=0)
        c2 = torch.stack([self.clusters[id]['feats3'] for id in env_id], dim=0) + \
             torch.stack([self.clusters_momentum[id]['feats3'] for id in env_id], dim=0)
        clusters = [c0, c1, c2]
    ...
    def aug_test(self, feats, img_metas, env_id=0, rescale=False):
        ...
        for x, img_meta, e_id in zip(feats, img_metas, env_id):
            ...
            img_name = [img_metas[0][0]['filename'] for _ in range(3)]
            c0 = torch.stack([self.clusters[id]['feats1'] for id in e_id], dim=0) + \
             torch.stack([self.clusters_momentum[id]['feats1'] for id in e_id], dim=0)
            c1 = torch.stack([self.clusters[id]['feats2'] for id in e_id], dim=0) + \
             torch.stack([self.clusters_momentum[id]['feats2'] for id in e_id], dim=0)
            c2 = torch.stack([self.clusters[id]['feats3'] for id in e_id], dim=0) + \
             torch.stack([self.clusters_momentum[id]['feats3'] for id in e_id], dim=0)
