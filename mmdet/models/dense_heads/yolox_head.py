...
class YOLOXHead(BaseDenseHead, BBoxTestMixin):
    ...
    def forward_single(self, x, cls_convs, reg_convs, conv_cls, conv_reg,
        ...
        """
        mean_x = torch.mean(x.squeeze(0), dim=0).detach().cpu().numpy()
        data_col_max, data_col_min = np.max(mean_x, axis=0), np.min(mean_x, axis=0)
        data_row = mean_x.shape[0]
        data_col = mean_x.shape[1]

        if mean_x.shape[0] == 80:
            self.num += 1
        
        for i in range(data_row):
            for j in range(data_col):
                mean_x[i][j] = (mean_x[i][j] - data_col_min[j]) / (data_col_max[j] - data_col_min[j])
        
        for i in range(data_row):
            for j in range(data_col):
                mean_x[i][j] = mean_x[i][j] * 255

        plt.savefig('/home/ml/code/mmdetection/newwork_out_yolox/{:0>12d}_{}_{}.png'.format(self.num, x.shape[2], x.shape[3]))
        plt.axis('off')
        plt.imshow(mean_x)"""
