...
model = dict(
    ...
    bbox_head=dict(
        ...
        in_channel2=100,
        cluster=dict(cityscapes='/benchmark/持续学习数据集/projects/few_shot/fewshot_fea/kitti/kitti_cluster.pth',
        foggy_cityscapes='/benchmark/持续学习数据集/projects/few_shot/fewshot_fea/kitti/city_cluster.pth',),
...
train_pipeline = [
    ...
    dict(
        type='CopyPaste',
        max_num_pasted=100,
        bbox_occluded_thr=15,
        mask_occluded_thr=350),
    dict(type='Mosaic', img_scale=img_scale, pad_val=114.0),
    dict(
        type='RandomAffine',
...
train_dataset = dict(
    ...
    datasets=[
        ...
        classes=classes,
        env_id=0,
        ann_file="/benchmark/持续学习数据集/public_data/kitti_COCO_format/annotations/only_car_instances_train2014_64.json",
        img_prefix="/benchmark/持续学习数据集/public_data/kitti_COCO_format/train2014/",
        pipeline=[
...
optimizer = dict(
    ...
    nesterov=True,
    paramwise_cfg=dict(
        custom_keys={'backbone': dict(lr_mult=0.8), 'neck': dict(lr_mult=0.8), 'norm': dict(decay_mult=0.), 'bias': dict(decay_mult=0.),
        'clus_m': dict(lr_mult=0.2)
        })
    )
...
resume_from = None
interval = 10
load_from = "/home/ml/code/mmdetection/work_dirs/yolox_fs_x_640x640_50e_kitti_pretrain_mom/best_0_bbox_mAP_epoch_50.pth"
